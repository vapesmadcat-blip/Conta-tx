...
function salvarNovoMotoristaMaster() {
    ...
    // Obter ponto do MASTER
    const pontoMaster = metadadosTurno.ponto || localStorage.getItem('driverflux_ponto_atual') || 'Ponto Padrão';
    const pontoId = pontoMaster.toLowerCase().replace(/\s+/g, '_');
    ...
    db.ref(`usuarios/${user}`).once('value').then(snapshot => {
        ...
        // Adicionar motorista à lista do ponto
        const pontoMaster = metadadosTurno.ponto || localStorage.getItem('driverflux_ponto_atual') || 'Ponto Padrão';
        const pontoId = pontoMaster.toLowerCase().replace(/\s+/g, '_');
        return db.ref(`pontos/${pontoId}/motoristas/${user}`).set({
            nome: user,
...
window.abrirModalCadastroPrefixo = abrirModalCadastroPrefixo;
window.fecharModalCadastroPrefixo = fecharModalCadastroPrefixo;
window.salvarNovoPrefixo = salvarNovoPrefixo;
window.abrirModalGerenciamentoPonto = abrirModalGerenciamentoPonto;
window.fecharModalGerenciamentoPonto = fecharModalGerenciamentoPonto;
window.salvarAlteracoesPonto = salvarAlteracoesPonto;
...
function abrirModalCadastroPrefixo() {
    if (usuarioLogado !== 'master') {
...
async function salvarNovoPrefixo() {
    ...
    try {
        ...
        await db.ref(`pontos/${pontoId}/carros/${prefixoId}`).set({
            placa: placa,
            modelo: modelo,
...
function abrirModalGerenciamentoPonto() {
    if (usuarioLogado !== 'master') {
        alert('🔒 Apenas o MASTER pode gerenciar o ponto.');
        return;
    }
